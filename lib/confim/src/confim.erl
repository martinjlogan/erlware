...
%% @equiv get_env(confim, Key, undefined)
get_env(Key) ->
...
modify_config_file([H|_] = FileLocation, AppGroup, Key, Val) when is_integer(H) ->
    case confim:get_env(AppGroup, Key) of
	undefined -> 
...
%%
%% Example: modify_config_value("/tmp/app.config", confim, spec, 
...
modify_config_value(FileLocation, AppGroup, Key, Fun) ->
    case confim:get_env(AppGroup, Key, undefined) of
	{ok, undefined} -> 
	    {error, no_such_config_entry};
	{ok, Value} ->
	    NewValue = Fun(Value),
	    confim:set_env(AppGroup, Key, NewValue),
